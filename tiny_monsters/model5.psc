Function:ATPcheck,ATP,cost,{piecewise(ATP,ge(ATP,cost),1)}

# basic gene parameters for X
# transcription rates
X_activated_transcription:
    X_activated_promoter + {2700} ATP > X_activated_promoter + X_mrna
    activated_transcription_rate * X_activated_promoter * ATP * piecewise(ATP,ge(ATP,2700),1)  
X_basal_transcription:
    X_basal_promoter + {2700} ATP > X_basal_promoter + X_mrna
    basal_transcription_rate * X_basal_promoter * ATP * piecewise(ATP,ge(ATP,2700),1)
X_repressed_transcription:
    X_repressed_promoter + {2700} ATP > X_repressed_promoter + X_mrna
    repressed_transcription_rate * X_repressed_promoter * ATP * piecewise(ATP,ge(ATP,2700),1)
# translation rates
X_translation:
    X_mrna + {1560} ATP > X_mrna + X_protein
    translation_rate * X_mrna * ATP * piecewise(ATP,ge(ATP,1560),1)
# degredation rates
X_mrna_deg:
    X_mrna > $pool
...
X_activates_P:
    ...
    0.1685123117 * X_protein * P_basal_promoter
X_unactivates_P:
...
T_activated_transcription:
    ...
    activated_transcription_rate * T_activated_promoter * ATP * piecewise(ATP,ge(ATP,2700),1)
T_basal_transcription:
    T_basal_promoter + {2700} ATP > T_basal_promoter + T_mrna
    basal_transcription_rate * T_basal_promoter * ATP * piecewise(ATP,ge(ATP,2700),1)
T_repressed_transcription:
    T_repressed_promoter + {2700} ATP > T_repressed_promoter + T_mrna
    repressed_transcription_rate * T_repressed_promoter * ATP * piecewise(ATP,ge(ATP,2700),1)
# translation rates
T_translation:
    T_mrna + {1560} ATP > T_mrna + T_protein
    translation_rate * T_mrna * ATP * piecewise(ATP,ge(ATP,1560),1)
# degredation rates
T_mrna_deg:
    T_mrna > $pool
...
P_activated_transcription:
    ...
    activated_transcription_rate * P_activated_promoter * ATP * piecewise(ATP,ge(ATP,2700),1)
P_basal_transcription:
    P_basal_promoter + {2700} ATP > P_basal_promoter + P_mrna
    basal_transcription_rate * P_basal_promoter * ATP * piecewise(ATP,ge(ATP,2700),1)
P_repressed_transcription:
    P_repressed_promoter + {2700} ATP > P_repressed_promoter + P_mrna
    repressed_transcription_rate * P_repressed_promoter * ATP * piecewise(ATP,ge(ATP,2700),1)
# translation rates
P_translation:
    P_mrna + {1560} ATP > P_mrna + P_protein
    translation_rate * P_mrna * ATP * piecewise(ATP,ge(ATP,1560),1)
# degredation rates
P_mrna_deg:
    P_mrna > $pool
